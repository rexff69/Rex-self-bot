export default {
  ...
  async execute(message, args, client) {
    ...
    if (!target.kickable) {
      await message.channel.send('❌ I cannot kick this user. Check my permissions.');
      return;
    }

    try {
      await target.kick(`Kicked by ${message.author.tag} via selfbot.`);
      await message.channel.send(`✅ ${target.user.tag} has been kicked.`);
      if (message.deletable) await message.delete().catch(() => {});
    } catch (err) {
      console.error('[Kick Error]:', err);
      await message.channel.send('❌ Failed to kick the member.');
    }
  },
};
